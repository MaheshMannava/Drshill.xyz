...
import { parseAbiItem, decodeEventLog, Abi } from "viem";

// Define the structure of the EventCreated event - IMPORTANT: Updated to match actual contract event signature
const eventCreatedAbiItem = parseAbiItem('event EventCreated(bytes32 indexed eventId, uint256 startTime, uint256 endTime, bytes32 qrCodeHash)');
const cropCircleAbi = [
  eventCreatedAbiItem,
      ...
      if (log.topics && log.topics.length > 0) {
        for (const topic of log.topics) {
          if (typeof topic === 'string' && topic.startsWith('0x') && topic.length === 66 && 
              topic !== eventCreatedSignature) { // Exclude the event signature itself
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle className="text-center">Permission Denied</DialogTitle></DialogHeader>
          <div className="p-6 text-center">
            <p className="text-red-500 mb-4">Only the contract owner can create events.</p>
            <Button onClick={() => onOpenChange(false)} className="w-full bg-gray-600 hover:bg-gray-700">Close</Button>
          </div>
        </DialogContent>
      </Dialog>
    );
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {displayStep === "form" && (
          <div className="space-y-4 pt-4">
            <div className="space-y-2">
              <label htmlFor="duration" className="block text-sm font-medium">Event Duration (hours)</label>
              <Input id="duration" type="number" min="1" value={duration} onChange={(e) => setDuration(e.target.value)} placeholder="24" className="w-full" />
            </div>
            <Button onClick={handleCreateEvent} disabled={isSubmitting} className="w-full bg-blue-600 hover:bg-blue-700">
        ...
        {displayStep === "pending" && (
          <div className="space-y-4 py-4 text-center">
            ...
            <div className="text-xs bg-gray-100 p-2 rounded break-all whitespace-nowrap overflow-hidden text-ellipsis">Tx Hash: {txHash}</div>
        ...
        {displayStep === "error_confirming" && (
          <div className="space-y-4 py-4 text-center">
            ...
            <div className="text-xs bg-gray-100 p-2 rounded break-all whitespace-nowrap overflow-hidden text-ellipsis">Tx Hash: {txHash}</div>
            <p className="text-xs text-red-500 mt-2">Please check your wallet for more details or try creating the event again.</p>
             <Button onClick={() => { setTxHash(undefined); setIsParsingLogs(false); }} variant="outline" size="sm" className="mt-4">Try Again</Button>
          </div>
        )}
        ...
        {displayStep === "complete" && confirmedEventUrl && confirmedEventId && (
          <div className="space-y-4 py-4 text-center">
            ...
            <div className="text-xs bg-gray-100 p-2 rounded break-all whitespace-nowrap overflow-hidden text-ellipsis">{confirmedEventUrl}</div>
            <p className="text-xs text-gray-500 mt-1 whitespace-nowrap overflow-hidden text-ellipsis">Event ID: {confirmedEventId}</p>
            <Button onClick={() => navigator.clipboard.writeText(confirmedEventUrl)} variant="outline" size="sm" className="mt-2">Copy Link</Button>
          </div>
        )}
      </DialogContent>
    </Dialog>
